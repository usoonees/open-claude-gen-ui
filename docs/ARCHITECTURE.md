...
- `scripts/codex-gen-ui-mcp.mjs` exposes the same `visualizeReadMe` and `showWidget` surface as a stdio MCP server for Codex CLI. The Codex adapter configures this MCP server when launching `codex app-server`, records Codex thread ids under `.data/codex-sessions/` so later app chat turns can resume the same Codex thread, maps app-server JSON-RPC notifications back into AI SDK UI chunks, and accepts explicit bridge tool-call blocks as a fallback when the Codex session lists the MCP server but does not surface its tools to the model. Native app-server deltas, MCP tool notifications, and bridge tool calls all feed the existing widget renderer through normal `tool-input-*` and `tool-output-*` UI chunks.
...
- `DEEPSEEK_API_KEY`, `DEEPSEEK_BASE_URL`, and `DEEPSEEK_MODEL` provide the env fallback and defaults for DeepSeek in the provider selector; new setups default to `deepseek-v4-pro`.
...
- `ANTHROPIC_API_KEY` and `ANTHROPIC_MODEL` provide the env fallback and default model for Anthropic in the provider selector; `ANTHROPIC_MODEL` is only accepted when it names a Claude model.
...
- `CHAT_AGENT_RUNTIME=codex` switches `/api/chat` from direct AI SDK provider calls to `codex app-server --listen stdio://`; unset or any other value keeps the provider-backed `ToolLoopAgent` path.
- `CODEX_APP_SERVER_MODEL` optionally selects the model for the Codex-backed chat runtime. `CODEX_EXEC_MODEL` is still accepted as a legacy fallback.
- `CODEX_APP_SERVER_SANDBOX` optionally selects the Codex app-server sandbox mode for the Codex-backed chat runtime. `CODEX_EXEC_SANDBOX` is still accepted as a legacy fallback.
