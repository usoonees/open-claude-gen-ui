...
- `lib/chat-agent.ts` defines the single-agent prompt, response-language matching rule, gen-ui behavior bias, and step limit, while `lib/chat-models.ts` owns the provider registry and model-list fetchers.
