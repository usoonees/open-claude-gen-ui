...
| Security | C | API keys stay server-only, and local provider plus Tavily keys can now be saved in encrypted `.data` stores for dev use; no auth or multi-user isolation is present. | Add auth, rate limiting, and production secret handling before public deployment. |
