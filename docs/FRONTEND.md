...
- In `Settings`, confirm the dialog stays compact, uses title-first cards without helper-description blocks, and still lets users save or remove a Tavily API key locally without restarting.
- In `Settings`, confirm trusted generative UI starts in the `On` state by default, users can toggle it on or off without restarting, and the switch stays compact without extra descriptive caption text.
...
- Switch between saved conversations and confirm the message pane shows a transcript skeleton while history loads, without flashing the empty new-chat headline or starter prompts.
...
- With `SHOWCASE_ONLY=true`, confirm the shared `/chat/:id` routes still render the same sidebar and composer shell as the live app, the six curated chats load from exported snapshots, the composer textarea shows a forbidden cursor and stays disabled, chat options and model selection remain locked, and `Settings` still opens as a compact read-only dialog with a GitHub source link and disabled mutation controls.
